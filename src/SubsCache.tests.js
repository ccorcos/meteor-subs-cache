...
describe("SubsCache.helpers", function() {
  ...
  describe("callbacksFromArgs", function() {
    it("extracts callbacks from args if existent", function() {
      ...
      assert.deepEqual(callbacksFromArgs.call(null, [null, cb]), {
        onReady: cb
      });
    });
...
describe("SubsCache - subscribe", function() {
  ...
  it("allows multiple subscriptions with same name", function() {
    var subsCache = new SubsCache();
    var sub = subsCache.subscribe(
      publicationAllDocuments,
      { _id: "01234567" },
      10,
      { sort: { _id: 1 } }
    );
    var hash = SubsCache.computeHash(
      publicationAllDocuments,
      { _id: "01234567" },
      10,
      { sort: { _id: 1 } }
    ...
    var sub2 = subsCache.subscribe(
      publicationAllDocuments,
      { _id: "01234567" },
      15,
      { sort: { _id: 1 } }
    );
    var hash2 = SubsCache.computeHash(
      publicationAllDocuments,
      { _id: "01234567" },
      15,
      { sort: { _id: 1 } }
